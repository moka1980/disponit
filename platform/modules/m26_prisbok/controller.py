"""Controlleren for m26_prisbok (ARC B tilbud, PR 4): claim → kontroll
→ bygg → send → signert kvittering. M-23/M-44/M-17-controllerens form —
mekanikken er delt (`modules.felles.levering`), dommene er tilbudets.

Det payloaden aldri bærer, og claim-svaret gir i `utforelse` (172):
kundens adresse dekryptert, navnet, linjene med registerets tall, den
bundne klausulteksten og avsenderprofilen. Modulen har verken KEK eller
base. Mangler feltet, eller bærer det en `hindring` (tilbudet ikke
lenger godkjent, utløpt, adresse uleselig), kvitteres `feilet` uten å
sende.
...
def http_frist_s() -> float:
    """Budsjettet per HTTP-kall innenfor tilbudets 15-minutters
    utførelsesfrist."""
    return 30.0
...
def kjor_en(klient, token: str, sender, signer) -> dict:
    ...
    if not isinstance(payload, dict):
        payload = {}
    tid = str(payload.get("tilbud_id") or "")
    basis = {
        ...
        "ressurs_id": f"tilbud:{tid}" if tid else "",
    }
    ...
    def nei(rk, grunn, **ekstra):
        # Journalen skal si HVILKET tilbud som ikke ble sendt.
        return feilutfall(rk, grunn, tilbud=tid, **ekstra)
