"""Inngangspunkt for `disponit-lonnssveip.service` (M-39, 113).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_LONNSSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_LONNSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "lonnssveip.json"
    return Path("/var/lib/disponit/lonnssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "lonnssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = lonnssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = lonnssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= lonnssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        ...
        # INGEN `avkortet`: M-39s sveip tar hele tenantlisten opp til
