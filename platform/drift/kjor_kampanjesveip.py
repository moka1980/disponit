"""Inngangspunkt for `disponit-kampanjesveip.service` (M-44, 114).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_KAMPANJESVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_KAMPANJESVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "kampanjesveip.json"
    return Path("/var/lib/disponit/kampanjesveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "kampanjesveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = kampanjesveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = kampanjesveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= kampanjesveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "kampanjesveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `avkortet`: M-44s sveip tar hele tenantlisten opp til
