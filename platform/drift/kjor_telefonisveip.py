"""Inngangspunkt for `disponit-telefonisveip.service` (M-43, 135).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_TELEFONISVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_TELEFONISVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "telefonisveip.json"
    return Path("/var/lib/disponit/telefonisveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "telefonisveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= telefonisveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_TELEFONISVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m43_sveip_telefoni()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "telefonisveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = telefonisveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = telefonisveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= telefonisveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "telefonisveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-43
        # har ingenting tilsvarende. En eskalering ingen tok ryddes
        # ikke av noen — den lukkes av at et menneske faktisk gjør noe
        # med den, og det er hele poenget med funnet.
        #
