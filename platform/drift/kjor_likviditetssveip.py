"""Inngangspunkt for `disponit-likviditetssveip.service` (M-15, 128).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_LIKVIDITETSSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_LIKVIDITETSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "likviditetssveip.json"
    return Path("/var/lib/disponit/likviditetssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "likviditetssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= likviditetssveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_LIKVIDITETSSVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "likviditetssveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = likviditetssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = likviditetssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= likviditetssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
