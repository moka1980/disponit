"""Inngangspunkt for `disponit-onboardingsveip.service` (M-18, 103).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_ONBOARDINGSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_ONBOARDINGSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "onboardingsveip.json"
    return Path("/var/lib/disponit/onboardingsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "onboardingsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = onboardingsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = onboardingsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= onboardingsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
