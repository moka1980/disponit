"""Inngangspunkt for `disponit-medarbeidersveip.service` (M-40, 140).

FLÅTENS SISTE KJØRESKRIPT. Formen er `kjor_transportsveip.py` sin,
ordrett.
...
overlevde begge feilene. Den ATOMISKE skrivingen er en del av formen:
et direkte skriv trunkerer den eneste persisterte telleren, og en
avbrutt kjøring i det vinduet leser den som 0 ved neste aktivering.

LINJEN SOM SKRIVES BÆRER ALDRI ET PULSSVAR. Den bærer fire tall —
tenanter, nye, oppdaterte, lukkede — og ingen av dem er en verdi noen
har avgitt. En driftslogg med pulsdata ville vært stedet anonymiteten
lakk ut, uten at noen hadde bedt om det.
"""
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_MEDARBEIDERSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_MEDARBEIDERSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "medarbeidersveip.json"
    return Path("/var/lib/disponit/medarbeidersveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "medarbeidersveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= medarbeidersveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_MEDARBEIDERSVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m40_sveip_medarbeider()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "medarbeidersveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = medarbeidersveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = medarbeidersveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= medarbeidersveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "medarbeidersveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-28
        # har ingenting tilsvarende — og det er hele poenget. En plan
        # ingen så på ryddes ikke av noen; den lukkes av at et
        # menneske faktisk forkaster den eller gjør noe med den.
        #
        # INGEN `bestillinger`: sveipen bestiller ingenting, og et felt
        # med verdien 0 ville vært en linje som lot som den talte.
        #
