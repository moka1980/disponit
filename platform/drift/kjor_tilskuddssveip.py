"""Inngangspunkt for `disponit-tilskuddssveip.service` (M-51, 119).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_TILSKUDDSSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_TILSKUDDSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "tilskuddssveip.json"
    return Path("/var/lib/disponit/tilskuddssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "tilskuddssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = tilskuddssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = tilskuddssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= tilskuddssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "tilskuddssveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-51
        # har ingenting tilsvarende — et uavklart treff ryddes ikke av
        # noen, det er hele poenget.
        #
        # INGEN `avkortet`: M-51s sveip tar hele tenantlisten opp til
