"""Inngangspunkt for `disponit-hmssveip.service` (M-53, 127).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_HMSSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_HMSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "hmssveip.json"
    return Path("/var/lib/disponit/hmssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "hmssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= hmssveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_HMSSVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "hmssveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = hmssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = hmssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= hmssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
