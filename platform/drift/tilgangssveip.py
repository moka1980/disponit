...
def kjor(conn, *, grense: int = GRENSE_PER_TENANT,
    ...
    try:
        try:
            rad = conn.execute("SELECT * FROM m12_sveip_gjennomganger(%s)",
                               (grense,)).fetchone()
            conn.commit()
        except Exception:
            _rull_tilbake(conn)
            ...
            return res
        # Døren returnerer nøyaktig én rad. Ingen rad er ikke «null funn»
        # — det er en dør som ikke oppførte seg som kontrakten, og da skal
        # kjøringen si feilet framfor å rapportere nuller den ikke har
        # målt («en jobb som ikke kunne måle rapporterer FUNN, aldri
        # null»).
        if rad is None:
            res.feilet = True
            res.alarm_utlost = tidligere_feil + 1 >= ALARM_ETTER_FEIL
