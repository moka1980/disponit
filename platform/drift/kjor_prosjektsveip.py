"""Inngangspunkt for `disponit-prosjektsveip.service` (M-25, 107).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_PROSJEKTSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_PROSJEKTSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "prosjektsveip.json"
    return Path("/var/lib/disponit/prosjektsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "prosjektsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = prosjektsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = prosjektsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= prosjektsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
