"""Inngangspunkt for `disponit-kontovaktsveip.service` (M-42, 110).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_KONTOVAKTSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_KONTOVAKTSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "kontovaktsveip.json"
    return Path("/var/lib/disponit/kontovaktsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "kontovaktsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = kontovaktsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = kontovaktsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= kontovaktsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
