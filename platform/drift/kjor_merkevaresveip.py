"""Inngangspunkt for `disponit-merkevaresveip.service` (M-55, 120).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_MERKEVARESVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_MERKEVARESVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "merkevaresveip.json"
    return Path("/var/lib/disponit/merkevaresveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "merkevaresveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    dsn = os.environ.get("DISPONIT_MERKEVARESVEIP_URL")
    if not dsn:
        # INGEN fallback til DATABASE_URL. Runtime-rollen har med vilje
        # ikke EXECUTE på sveipen (120 REVOKEr den), så en fallback ville
        ...
        print(json.dumps({
            "hendelse": "merkevaresveip", "tenanter": 0,
            "nye_funn": 0, "oppdaterte_funn": 0, "lukkede_funn": 0,
            "feilet": 1, "hoppet_over": 0, "sammenhengende_feil": n,
            "alarm": int(n >= merkevaresveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_MERKEVARESVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m55_sveip_merkevare()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "merkevaresveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = merkevaresveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = merkevaresveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= merkevaresveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "merkevaresveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-55
        # har ingenting tilsvarende — et uavklart treff ryddes ikke av
        # noen, det er hele poenget.
        #
        # INGEN `avkortet`: M-55s sveip tar hele tenantlisten opp til
