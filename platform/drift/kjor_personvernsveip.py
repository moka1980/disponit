"""Inngangspunkt for `disponit-personvernsveip.service` (M-30, 099).
...
from . import personvernsveip


def _tilstandsfil() -> Path:
    """Hvor feiltelleren ligger.

    Unit-filen setter `StateDirectory=disponit`, så systemd
    oppretter katalogen med arbeiderens egen eier FØR ExecStart og
    oppgir den i $STATE_DIRECTORY. Den hardkodede stien beholdes kun som
    fallback for kjøring utenfor systemd; $DISPONIT_PERSONVERNSVEIPTILSTAND
    overstyrer alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_PERSONVERNSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "personvernsveip.json"
    return Path("/var/lib/disponit/personvernsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "personvernsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = personvernsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = personvernsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= personvernsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
