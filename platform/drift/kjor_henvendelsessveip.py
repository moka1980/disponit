"""Inngangspunkt for `disponit-henvendelsessveip.service` (M-13, 101).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_HENVENDELSESVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_HENVENDELSESVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "henvendelsessveip.json"
    return Path("/var/lib/disponit/henvendelsessveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "henvendelsessveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = henvendelsessveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = henvendelsessveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= henvendelsessveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "henvendelsessveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # Ikke en feil, men heller ikke «ferdig»: sveipen traff taket sitt
        # hos så mange tenanter, og neste kjøring tar igjen resten.
        "avkortet": r.avkortet,
        "feilet": int(r.feilet),
