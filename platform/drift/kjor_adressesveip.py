"""Inngangspunkt for `disponit-adressesveip.service` (M-19, 112).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_ADRESSESVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_ADRESSESVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "adressesveip.json"
    return Path("/var/lib/disponit/adressesveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "adressesveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = adressesveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = adressesveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= adressesveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "adressesveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `avkortet`: M-19s sveip tar hele tenantlisten opp til
