"""Inngangspunkt for `disponit-motpartssveip.service` (M-48, 116).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_MOTPARTSSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_MOTPARTSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "motpartssveip.json"
    return Path("/var/lib/disponit/motpartssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "motpartssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m48_sveip_motparter()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "motpartssveip", "tenanter": 0, "nye_funn": 0,
            "oppdaterte_funn": 0, "lukkede_funn": 0, "forlatte": 0,
            "feilet": 1, "hoppet_over": 0, "sammenhengende_feil": n,
            "alarm": int(n >= motpartssveip.ALARM_ETTER_FEIL),
    ...
    try:
        r = motpartssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = motpartssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= motpartssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "motpartssveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # FORLATTE STÅR FOR SEG. Det er den eneste raden sveipen endrer
        # utenfor funntabellen, og en natt der tjue oppslag ble forlatt
        # ser ellers ut som en helt vanlig natt. Tallet er dessuten den
        # billigste måten å oppdage en klient som dør systematisk
        # mellom reservasjon og fullføring.
        "forlatte": r.forlatte,
        # INGEN `avkortet`: M-48s sveip tar hele tenantlisten opp til
        # taket og har ikke noe per-tenant-tak å rapportere. Å bære med
        # seg feltet fra 111 med verdien 0 ville vært en linje som lot
        # som den målte noe.
        "feilet": int(r.feilet),
