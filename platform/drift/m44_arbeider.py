"""M-44-arbeideren: claimer kampanje.send-oppdrag over API-et med
modultokenet fra onboarding, leverer e-posten gjennom husets SMTP, og
...
og teksten kommer i claim-svaret (156) og lever i minnet så lenge
sendingen varer.

Miljø (EnvironmentFile /etc/disponit/m44/konfig + LoadCredential):
  ...
  DISPONIT_KVITTERINGSNOKKEL sti til JSON {verifikator, nokkel_id,
                             hemmelighet} — kampanjens er `v_samtykke`
  DISPONIT_SMTP_VERT/PORT/BRUKER/PASSORD/AVSENDER  husets SMTP
  DISPONIT_M44_POLL_S        pause mellom tomme claim (standard 10)
"""
...
def main() -> int:
    ...
    poll_s = float(os.environ.get("DISPONIT_M44_POLL_S", "10"))
    print(json.dumps({"hendelse": "m44_arbeider_oppe", "api": api}),
          flush=True)
    while True:
        try:
        ...
        print(json.dumps({"hendelse": "oppdrag_behandlet", **{
            ...
            if k in ("utfall", "grunn", "kvittering_status", "kampanje",
                     "mottaker", "feiltype")}}), flush=True)
    return 0
