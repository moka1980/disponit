"""Inngangspunkt for `disponit-skattesveip.service` (M-32, 138).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_SKATTESVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_SKATTESVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "skattesveip.json"
    return Path("/var/lib/disponit/skattesveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "skattesveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= skattesveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_SKATTESVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m32_sveip_skatt()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "skattesveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = skattesveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = skattesveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= skattesveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "skattesveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-32
        # har ingenting tilsvarende — og det er hele poenget. En stor
        # vurdering ingen så på ryddes ikke av noen; den lukkes av at
        # et menneske faktisk kontrollerer den.
        #
        # INGEN `innberetninger`: sveipen sender ingenting, og et felt
