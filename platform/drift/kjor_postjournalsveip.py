"""Inngangspunkt for `disponit-postjournalsveip.service` (M-50, 124).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_POSTJOURNALSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_POSTJOURNALSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "postjournalsveip.json"
    return Path("/var/lib/disponit/postjournalsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "postjournalsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= postjournalsveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_POSTJOURNALSVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "postjournalsveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = postjournalsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = postjournalsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= postjournalsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
