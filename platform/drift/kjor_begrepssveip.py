"""Inngangspunkt for `disponit-begrepssveip.service` (M-9, 095).
...
def _tilstandsfil() -> Path:
    ...
    fallback for kjøring utenfor systemd; $DISPONIT_BEGREPSSVEIPTILSTAND
    overstyrer alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_BEGREPSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "begrepssveip.json"
    return Path("/var/lib/disponit/begrepssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "begrepssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = begrepssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = begrepssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= begrepssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
