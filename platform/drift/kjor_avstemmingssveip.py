"""Inngangspunkt for `disponit-avstemmingssveip.service` (M-13, 101).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_AVSTEMMINGSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_AVSTEMMINGSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "avstemmingssveip.json"
    return Path("/var/lib/disponit/avstemmingssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "avstemmingssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = avstemmingssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = avstemmingssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= avstemmingssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
