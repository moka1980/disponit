"""Inngangspunkt for `disponit-tollkodesveip.service` (M-52, 122).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_TOLLKODESVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_TOLLKODESVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "tollkodesveip.json"
    return Path("/var/lib/disponit/tollkodesveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "tollkodesveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= tollkodesveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_TOLLKODESVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m52_sveip_tollkode()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "tollkodesveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = tollkodesveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = tollkodesveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= tollkodesveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
