"""M-18 (103) — onboardingsveipen for kundeløpene.

`disponit-onboardingsveip.timer`, én gang i døgnet, kaller
`m18_sveip_onboarding(p_grense, p_dogn_stille)`.

Sveipen legger INGEN logikk oppå regelen. Regelen eies av databasen
(103): et pågående løp uten framdrift på lenge er et `stoppet_lop`-funn;
et ufullført steg forbi sin egen frist er `steg_over_frist`; og et
pågående løp hvis eier ikke lenger er aktivt medlem er
`lop_uten_aktiv_eier`. Timeren bestemmer ikke hva som står stille; den
kaller funksjonen som vet det.

STILLE MÅLES FRA SISTE FULLFØRING, ikke fra starten. Uten det ville et
løp der noen jobbet i går blitt et funn fordi det ble startet for en
måned siden — og funnlisten ville fylt seg med løp som går som de skal.

SVEIPEN PROVISJONERER INGENTING. Den kaller én definer og får fem tall
tilbake. Katalogteksten lover 0 minutter per ny kunde; v1 registrerer
løpet, og denne filen er et av stedene fraværet er lettest å se.

Formen er `henvendelsessveip.py` sin, ordrett, og av de samme grunnene:
  ...
  * **To sammenhengende feilede kjøringer → alarm.** En stille
    onboardingsveip er en ny kunde som blir liggende uten at noen ser
    det.
...
GRENSE = 500
#: Døgnene et PÅGÅENDE løp kan stå uten framdrift før det er et funn.
#: Fjorten døgn er to uker uten bevegelse på en ny kunde. Tallet står HER
#: og ikke i dørens kropp fordi det er en driftsvurdering og ikke en
#: invariant: en tenant med tettere oppfølging vil ha noe annet, og den
#: dagen dette blir en policyverdi (M-1), er endringen ett kall.
DOGN_STILLE = 14
...
def kjor(conn, *, grense: int = GRENSE,
    ...
    try:
        ...
        try:
            rader = conn.execute(
                "SELECT * FROM m18_sveip_onboarding(%s, %s)",
                (grense, dogn_stille)).fetchall()
        except Exception:
            _rull_tilbake(conn)
            res.feilet = True
