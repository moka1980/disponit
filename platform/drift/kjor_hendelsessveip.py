"""Inngangspunkt for `disponit-hendelsessveip.service` (M-29, 137).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_HENDELSESSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_HENDELSESSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "hendelsessveip.json"
    return Path("/var/lib/disponit/hendelsessveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "hendelsessveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= hendelsessveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_HENDELSESSVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m29_sveip_hendelse()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "hendelsessveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = hendelsessveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = hendelsessveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= hendelsessveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "hendelsessveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-29
        # har ingenting tilsvarende — og det er hele poenget. En
        # hendelse ingen så på ryddes ikke av noen; den lukkes av at et
        # menneske faktisk gjør noe med den.
        #
        # INGEN `inngrep`: sveipen kan ikke utføre noe, og et felt med
        # verdien 0 ville vært en linje som lot som den talte.
        #
