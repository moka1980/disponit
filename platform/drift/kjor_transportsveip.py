"""Inngangspunkt for `disponit-transportsveip.service` (M-28, 139).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_TRANSPORTSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_TRANSPORTSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "transportsveip.json"
    return Path("/var/lib/disponit/transportsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "transportsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= transportsveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_TRANSPORTSVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m28_sveip_transport()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "transportsveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = transportsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = transportsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= transportsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
