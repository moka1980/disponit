"""Inngangspunkt for `disponit-fordringssveip.service` (M-23, 104).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_FORDRINGSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_FORDRINGSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "fordringssveip.json"
    return Path("/var/lib/disponit/fordringssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "fordringssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = fordringssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = fordringssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= fordringssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
