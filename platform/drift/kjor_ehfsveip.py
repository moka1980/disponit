"""Inngangspunkt for `disponit-ehfsveip.service` (M-54, 121).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_EHFSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_EHFSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "ehfsveip.json"
    return Path("/var/lib/disponit/ehfsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "ehfsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    dsn = os.environ.get("DISPONIT_EHFSVEIP_URL")
    if not dsn:
        # INGEN fallback til DATABASE_URL. Runtime-rollen har med vilje
        # ikke EXECUTE på sveipen (121 REVOKEr den), så en fallback ville
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m54_sveip_ehf()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "ehfsveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = ehfsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = ehfsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= ehfsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        ...
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-54
        # har ingenting tilsvarende — et uavklart treff ryddes ikke av
        # noen, det er hele poenget.
        #
        # INGEN `avkortet`: M-54s sveip tar hele tenantlisten opp til
