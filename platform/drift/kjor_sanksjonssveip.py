"""Inngangspunkt for `disponit-sanksjonssveip.service` (M-49, 117).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_SANKSJONSSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_SANKSJONSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "sanksjonssveip.json"
    return Path("/var/lib/disponit/sanksjonssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "sanksjonssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = sanksjonssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = sanksjonssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= sanksjonssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        "hendelse": "sanksjonssveip",
        "tenanter": r.tenanter,
        "nye_funn": r.nye,
        "oppdaterte_funn": r.oppdaterte,
        "lukkede_funn": r.lukkede,
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-49
        # har ingenting tilsvarende — et uavklart treff ryddes ikke av
        # noen, det er hele poenget.
        #
        # INGEN `avkortet`: M-49s sveip tar hele tenantlisten opp til
