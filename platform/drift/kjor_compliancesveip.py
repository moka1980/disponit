"""Inngangspunkt for `disponit-compliancesveip.service` (M-34, 100).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_COMPLIANCESVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_COMPLIANCESVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "compliancesveip.json"
    return Path("/var/lib/disponit/compliancesveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "compliancesveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = compliancesveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = compliancesveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= compliancesveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
