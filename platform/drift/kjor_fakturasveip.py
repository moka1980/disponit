"""Inngangspunkt for `disponit-fakturasveip.service` (M-14, 106).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_FAKTURASVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_FAKTURASVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "fakturasveip.json"
    return Path("/var/lib/disponit/fakturasveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "fakturasveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = fakturasveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = fakturasveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= fakturasveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
