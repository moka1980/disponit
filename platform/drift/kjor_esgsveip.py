"""Inngangspunkt for `disponit-esgsveip.service` (M-45, 136).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_ESGSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_ESGSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "esgsveip.json"
    return Path("/var/lib/disponit/esgsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "esgsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= esgsveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_ESGSVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m45_sveip_esg()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "esgsveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = esgsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = esgsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= esgsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    print(json.dumps({
        ...
        # INGEN `forlatte`: M-48 rydder forlatte reservasjoner, M-45
        # har ingenting tilsvarende. Et estimat som ikke ble erstattet
        # ryddes ikke av noen — det lukkes av at noen faktisk måler
        # tallet, og det er hele poenget med funnet.
        #
