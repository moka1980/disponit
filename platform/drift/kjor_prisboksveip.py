"""Inngangspunkt for `disponit-prisboksveip.service` (M-26, 108).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_PRISBOKSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_PRISBOKSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "prisboksveip.json"
    return Path("/var/lib/disponit/prisboksveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "prisboksveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = prisboksveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = prisboksveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= prisboksveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
