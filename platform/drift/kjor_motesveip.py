"""Inngangspunkt for `disponit-motesveip.service` (M-7, 133).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_MOTESVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_MOTESVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "motesveip.json"
    return Path("/var/lib/disponit/motesveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "motesveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    if not dsn:
        ...
        print(json.dumps({
            ...
            "alarm": int(n >= motesveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_MOTESVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m7_sveip_moter()`,
        # ellers utløser en vedvarende tilkoblingsfeil aldri alarmen den
        # skal.
        n = tidligere + 1
        lagret = _skriv_feiltelling(n)
        print(json.dumps({
            "hendelse": "motesveip", "tenanter": 0, "nye_funn": 0,
    ...
    try:
        r = motesveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = motesveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= motesveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
