"""Inngangspunkt for `disponit-leverandorsveip.service` (M-24, 105).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_LEVERANDORSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_LEVERANDORSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "leverandorsveip.json"
    return Path("/var/lib/disponit/leverandorsveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "leverandorsveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = leverandorsveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = leverandorsveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= leverandorsveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
