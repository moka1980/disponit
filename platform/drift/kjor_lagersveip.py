"""Inngangspunkt for `disponit-lagersveip.service` (M-27, 109).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_LAGERSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_LAGERSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "lagersveip.json"
    return Path("/var/lib/disponit/lagersveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "lagersveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = lagersveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = lagersveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= lagersveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
