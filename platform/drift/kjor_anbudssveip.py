"""Inngangspunkt for `disponit-anbudssveip.service` (M-46, 118).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_ANBUDSSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_ANBUDSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "anbudssveip.json"
    return Path("/var/lib/disponit/anbudssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "anbudssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    dsn = os.environ.get("DISPONIT_ANBUDSSVEIP_URL")
    if not dsn:
        # INGEN fallback til DATABASE_URL. Runtime-rollen har med vilje
        # ikke EXECUTE på sveipen (118 REVOKEr den), så en fallback ville
        # bare byttet en tydelig oppstartsnekt mot «permission denied» i
        # journalen hver natt.
        # EN MANGLENDE DSN ER EN FEILET KJØRING (CodeRabbit, 118).
        ...
        print(json.dumps({
            "hendelse": "anbudssveip", "tenanter": 0, "nye_funn": 0,
            "oppdaterte_funn": 0, "lukkede_funn": 0,
            "feilet": 1, "hoppet_over": 0, "sammenhengende_feil": n,
            "alarm": int(n >= anbudssveip.ALARM_ETTER_FEIL),
            "tilstand_lagret": int(lagret),
            "grunn": "DISPONIT_ANBUDSSVEIP_URL mangler",
        }))
        return 2
    ...
    try:
        ...
        # øke akkurat som ved en feilet `m46_sveip_anbud()`,
    ...
    try:
        r = anbudssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = anbudssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= anbudssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
