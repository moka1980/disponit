"""M-17-arbeideren: claimer kundeservice.svar.send-oppdrag over API-et med
modultokenet fra onboarding, sender svaret gjennom husets SMTP, og
...
og teksten kommer i claim-svaret (163) og lever i minnet så lenge
sendingen varer.

Miljø (EnvironmentFile /etc/disponit/m17/konfig + LoadCredential):
  ...
  DISPONIT_KVITTERINGSNOKKEL sti til JSON {verifikator, nokkel_id,
                             hemmelighet} — svarets er `v_kundeservice`
  DISPONIT_SMTP_VERT/PORT/BRUKER/PASSORD/AVSENDER  husets SMTP
  DISPONIT_M17_POLL_S        pause mellom tomme claim (standard 10)
"""
...
def main() -> int:
    ...
    poll_s = float(os.environ.get("DISPONIT_M17_POLL_S", "10"))
    print(json.dumps({"hendelse": "m17_arbeider_oppe", "api": api}),
          flush=True)
    while True:
        try:
