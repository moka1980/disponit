"""Inngangspunkt for `disponit-betalingssveip.service` (M-41, 111).
...
def _tilstandsfil() -> Path:
    ...
    kjøring utenfor systemd; $DISPONIT_BETALINGSSVEIPTILSTAND overstyrer
    alt (tester, manuell drift).
    """
    eksplisitt = os.environ.get("DISPONIT_BETALINGSSVEIPTILSTAND")
    if eksplisitt:
    ...
    if statedir:
        ...
        return Path(statedir.split(":")[0]) / "betalingssveip.json"
    return Path("/var/lib/disponit/betalingssveip.json")
...
def _skriv_feiltelling(n: int) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"hendelse": "betalingssveiptilstand_skrivefeil",
...
def main() -> int:
    ...
    try:
        r = betalingssveip.kjor(conn, tidligere_feil=tidligere)
    except Exception:
        ...
        # mot.
        r = betalingssveip.Sveipresultat(
            feilet=True,
            alarm_utlost=tidligere + 1 >= betalingssveip.ALARM_ETTER_FEIL)
    finally:
        try:
            conn.close()
        except Exception:
