...
def test_p1_credentials_materialiseres_mot_en_fersk_rot(tmp_path):
    ...
    env.update({n: f"verdi-{n}" for n in (
        ...
        # 097 (M-12): gjennomgangssveipens EGEN DSN, samme klasse og
        ...
        # 100 (M-34): etterprøvingssveipens EGEN DSN, av samme grunn —
        # samme form, samme ene EXECUTE, samme fravær av
        # tabellrettigheter.
        "DISPONIT_COMPLIANCESVEIP_URL",
        # 101 (M-13): avstemmingssveipens EGEN DSN, av samme grunn —
        # samme form, samme ene EXECUTE, samme fravær av
        # tabellrettigheter.
        "DISPONIT_AVSTEMMINGSVEIP_URL",
        # 102 (M-17): henvendelsessveipens EGEN DSN, av samme grunn.
        "DISPONIT_HENVENDELSESVEIP_URL",
        # 103 (M-18): onboardingsveipens EGEN DSN, av samme grunn.
        "DISPONIT_ONBOARDINGSVEIP_URL",
        ...
        # 107 (M-25): prosjektsveipens EGEN DSN, av samme grunn.
        "DISPONIT_PROSJEKTSVEIP_URL",
        # 108 (M-26): prisboksveipens EGEN DSN, av samme grunn.
        "DISPONIT_PRISBOKSVEIP_URL",
        # 109 (M-27): lagersveipens EGEN DSN, av samme grunn.
        "DISPONIT_LAGERSVEIP_URL",
        # 110 (M-42): kontovaktsveipens EGEN DSN, av samme grunn.
        "DISPONIT_KONTOVAKTSVEIP_URL",
        ...
        # 114 (M-44): kampanjesveipens EGEN DSN, av samme grunn.
        "DISPONIT_KAMPANJESVEIP_URL",
        # 116 (M-48): motpartssveipens EGEN DSN, av samme grunn.
        "DISPONIT_MOTPARTSSVEIP_URL",
        ...
        # 140 (M-40): medarbeidersveipens EGEN DSN. En stille sveip her
        # er en maaling ingen faar lese og en ansatt ingen tok imot.
    ...
    assert (rot / "compliancesveip/DISPONIT_COMPLIANCESVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_COMPLIANCESVEIP_URL"
    assert (rot / "avstemmingssveip/DISPONIT_AVSTEMMINGSVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_AVSTEMMINGSVEIP_URL"
    assert (rot / "henvendelsessveip/DISPONIT_HENVENDELSESVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_HENVENDELSESVEIP_URL"
    assert (rot / "onboardingsveip/DISPONIT_ONBOARDINGSVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_ONBOARDINGSVEIP_URL"
    assert (rot / "fordringssveip/DISPONIT_FORDRINGSVEIP_URL").read_text(
    ...
    assert (rot / "prosjektsveip/DISPONIT_PROSJEKTSVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_PROSJEKTSVEIP_URL"
    assert (rot / "prisboksveip/DISPONIT_PRISBOKSVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_PRISBOKSVEIP_URL"
    assert (rot / "lagersveip/DISPONIT_LAGERSVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_LAGERSVEIP_URL"
    assert (rot / "kontovaktsveip/DISPONIT_KONTOVAKTSVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_KONTOVAKTSVEIP_URL"
    assert (rot / "betalingssveip/DISPONIT_BETALINGSSVEIP_URL").read_text(
    ...
    assert (rot / "kampanjesveip/DISPONIT_KAMPANJESVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_KAMPANJESVEIP_URL"
    # 116 (M-48): motpartssveipens EGEN DSN — klyngens ene utgående
    # kanal kjører med en rolle som har nøyaktig én EXECUTE.
    assert (rot / "motpartssveip/DISPONIT_MOTPARTSSVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_MOTPARTSSVEIP_URL"
    # 117 (M-49): sanksjonssveipens EGEN DSN — en stille sanksjonssveip
    # er uavklarte treff ingen har sett på.
    assert (rot / "sanksjonssveip/DISPONIT_SANKSJONSSVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_SANKSJONSSVEIP_URL"
    # 118 (M-46): anbudssveipens EGEN DSN — en stille anbudssveip er
    ...
    # 120 (M-55): merkevaresveipens EGEN DSN — en stille
    # merkevaresveip er forvekslinger ingen har sett på.
    assert (rot / "merkevaresveip/DISPONIT_MERKEVARESVEIP_URL").read_text(
        encoding="utf-8") == "verdi-DISPONIT_MERKEVARESVEIP_URL"
    # 121 (M-54): EHF-sveipens EGEN DSN — en stille EHF-sveip er en
    ...
    # 130 (M-33): prognosesveipens EGEN DSN.
