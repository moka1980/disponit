...
#: M-17 SERTIFISERING (16/9) — samme lest som M-23 (14/9). Andelen er
#: FILER modulen eier i sin helhet, av samme grunn som m23s: en liste
#: over node-id-er ville vært brutt av den første testen noen legger til.
M17_SUITE_ANDEL: tuple[str, ...] = (
    "platform/core/tests/test_epost_til_kundeservice_port.py",
    "platform/core/tests/test_m17_avsender_port.py",
    "platform/core/tests/test_m17_bokforing_port.py",
    "platform/core/tests/test_m17_controller.py",
    "platform/core/tests/test_m17_fasit_port.py",
    "platform/core/tests/test_m17_flate_port.py",
    "platform/core/tests/test_m17_kundeservice.py",
    "platform/core/tests/test_m17_sending_port.py",
    "platform/core/tests/test_m17_stilleregler_port.py",
    "platform/core/tests/test_m17_svarutloser_port.py",
)

#: M-17 FLIPPEDRILLEN, registrert FØR drillen er kjørt (§0), i
#: `rollback-m56-v1`s form som `m23-rollback-v1`: M-17 er en registrert
#: modul med én release (`m17-r1`) og en arbeider (`disponit-m17.service`)
#: som claimer oppdrag over API-et med modultoken. Punktet er BLOKKERT
#: til modulen har en forgjenger å rulle tilbake til — samme dom som
#: m23. Skjemaet og porten følger med drillen.
KRAVGRENSER["m17-rollback-v1"] = {
    "maks_claims_etter_drenering": 0,
    "min_inflight": 1,
    "maks_falske_verdikter": 0,
    "min_rullback_claims": 1,
...
KRAVGRENSER["m17-suite-v1"] = {
    "min_tester": 1500,
    "maks_feilet": 0,
    # Andelens gulv er MÅLT: 76 tester i de ti filene, null hoppede
    # (lokalt 16/9, med fasitportens fire). Gulvet står litt under, så en fjernet test ikke
    # feller punktet, men en HALVERT andel gjør det.
    "min_m17_tester": 65,
    "maks_m17_feilet": 0,
    # NULL HOPPEDE: hele andelen er `skipif(not DSN)`, og fasitporten
...
M17_SETT_STI = REPOROT / "deploy/staging/m17_fasit.py"

#: Produsentflaten for M-17s fasitartefakt. SVEIPEDRIVEREN OG REGELRUNDEN
#: ER MED: funnene er skrevet av `henvendelsessveip.kjor()` og dommene av
#: `stilleregler.kjor_en_runde()`, så en kjøring med en annen av dem er
#: en annen kjøring.
M17_BEVISROT_FILER = (
    "deploy/staging/m17_fasit.py",
    "deploy/staging/m17-fasit-artefakt.py",
    "deploy/staging/m17-suite-artefakt.py",
    "platform/drift/henvendelsessveip.py",
    "platform/drift/kjor_henvendelsessveip.py",
    "platform/core/plan/stilleregler.py",
)

KRAVGRENSER["m17-fasit-v1"] = {
    # SETTET ER EKSAKT: 21 henvendelser, 9 ventede funn, 3 regeldommer,
    # 61 evidenshendelser — nøyaktig de radene `m17_fasit.py` bærer, og
    # porten `test_settet_er_fasiten` leser tallene fra settet.
    #
    # TALLET 9 ER TELT: sju i regel-tenanten (uklassifisert på 3 og 10
    # døgn, ubesvart på 6, mistenkelig på 0, det lignende domenet, den
    # andre adressen, og menneskets `svar_kreves` på en stille avsender)
    # pluss to i den regelløse (den stille avsenderen uklassifisert, og
    # ubesvart på 6).
    "henvendelser_eksakt": 21,
    "ventede_funn_eksakt": 9,
    "regelklassifisert_eksakt": 3,
    "evidenshendelser_eksakt": 61,
    "maks_funnavvik": 0,
    "maks_klassifiseringsavvik": 0,
    "maks_koeavvik": 0,
    "maks_evidensavvik": 0,
    "krev_sett_sha_lik_innsjekket": True,
...
#: M-6 SERTIFISERING (16/9) — samme lest som M-17/M-23. Andelen er FILER
#: modulen eier i sin helhet.
M6_SUITE_ANDEL: tuple[str, ...] = (
    "platform/core/tests/test_epost_til_kundeservice_port.py",
    "platform/core/tests/test_m6_epost.py",
    "platform/core/tests/test_m6_fasit_port.py",
    "platform/core/tests/test_m6_inntak_grense_port.py",
    "platform/core/tests/test_m6_inntak_port.py",
    "platform/core/tests/test_m6_kilde.py",
    "platform/core/tests/test_m6_meldinger_port.py",
    "platform/core/tests/test_m6_slett_port.py",
    "platform/core/tests/test_m6_svar_grense_port.py",
    "platform/core/tests/test_m6_svarutkast_port.py",
...
KRAVGRENSER["m6-suite-v1"] = {
    "min_tester": 1500,
    "maks_feilet": 0,
    # Andelens gulv er MÅLT: 81 tester i de ti eksisterende filene pluss
    # fasitporten (16/9). Gulvet står litt under.
    "min_m6_tester": 75,
    "maks_m6_feilet": 0,
    # NULL HOPPEDE: andelen er `skipif(not DSN)`, og fasitporten krever
    # i tillegg planarbeiderens egen DSN.
    "maks_m6_hoppet": 0,
    "m6_andel_pakrevd": M6_SUITE_ANDEL,
    "punktbinding": {
        "tester_gronne_pa_staging": (
            "maalt.m6_feilet",
            "maalt.m6_tester",
...
M6_SETT_STI = REPOROT / "deploy/staging/m6_fasit.py"

#: Produsentflaten for M-6s fasitartefakt. INNHENTEREN OG LESE-API-ET ER
#: MED: radene er skrevet av `plan/epost.py` og lest av
#: `api/epost_meldinger.meldinger_for`, så en kjøring med en annen av
#: dem er en annen kjøring.
M6_BEVISROT_FILER = (
    "deploy/staging/m6_fasit.py",
    "deploy/staging/m6-fasit-artefakt.py",
    "deploy/staging/m6-suite-artefakt.py",
    "platform/core/plan/epost.py",
    "platform/core/api/epost_meldinger.py",
)

KRAVGRENSER["m6-fasit-v1"] = {
    # SETTET ER EKSAKT: seks meldinger per tenant (av sju sett; én
    # @removed og to duplikater), fem nye i runde 1 og én i runde 2, én
    # henvendelse per melding, ett svar sendt (tenanten med sendescope),
    # 28 evidenshendelser — nøyaktig det `m6_fasit.py` bærer, lest av
    # settet i `test_settet_er_fasiten`.
    "meldinger_eksakt": 12,
    "nye_runde1_eksakt": 10,
    "nye_runde2_eksakt": 2,
    "henvendelser_eksakt": 12,
    "sendt_eksakt": 1,
    "evidenshendelser_eksakt": 28,
    "maks_inntaksavvik": 0,
    "maks_broavvik": 0,
    "maks_svaravvik": 0,
    "maks_evidensavvik": 0,
    "krev_sett_sha_lik_innsjekket": True,
    # YTELSE: runde 1 av inntaket for begge postboksene, og per melding.
    # Taket er romslig med vilje — det som måles er at kostnaden ikke
    # EKSPLODERER; nevneren er `meldinger`.
    "maks_innhentingstid_ms": 30_000,
    "maks_innhentingstid_ms_per_melding": 5_000,
    "punktbinding": {
        "syntetisk_datasett_likt_lokalt": (
            "maalt.meldinger",
            "maalt.nye_runde1",
        ...
        "ytelse_bestatt": (
            "maalt.innhentingstid_ms",
            "maalt.meldinger",
        ),
    },
}

#: M-44 SERTIFISERING (16/9, natt) — samme lest som M-17/M-6/M-23.
M44_SUITE_ANDEL: tuple[str, ...] = (
    "platform/core/tests/test_bestilling_kampanje_port.py",
    "platform/core/tests/test_m44_bokforing_port.py",
    "platform/core/tests/test_m44_controller.py",
    "platform/core/tests/test_m44_fasit_port.py",
    "platform/core/tests/test_m44_flate_port.py",
    "platform/core/tests/test_m44_kampanje.py",
    "platform/core/tests/test_m44_kampanjeutloser_port.py",
    "platform/core/tests/test_m44_kontakt_port.py",
    "platform/core/tests/test_m44_samtykke_i_doera_port.py",
    "platform/core/tests/test_m44_sending_port.py",
)

#: M-44 FLIPPEDRILLEN (§0, `rollback-m56-v1`s form): M-44 har én release
#: (`m44-r1`) og en arbeider (`disponit-m44.service`). Blokkert til
#: modulen har en forgjenger.
KRAVGRENSER["m44-rollback-v1"] = {
    "maks_claims_etter_drenering": 0,
    "min_inflight": 1,
    "maks_falske_verdikter": 0,
    "min_rullback_claims": 1,
...
KRAVGRENSER["m44-suite-v1"] = {
    "min_tester": 1500,
    "maks_feilet": 0,
    # Andelens gulv er MÅLT: 78 tester i de ni eksisterende filene pluss
    # fasitportens fire (16/9). Gulvet står litt under.
    "min_m44_tester": 72,
    "maks_m44_feilet": 0,
    "maks_m44_hoppet": 0,
    "m44_andel_pakrevd": M44_SUITE_ANDEL,
    "punktbinding": {
        "tester_gronne_pa_staging": (
            "maalt.m44_feilet",
            "maalt.m44_tester",
...
M44_SETT_STI = REPOROT / "deploy/staging/m44_fasit.py"

#: Produsentflaten for M-44s fasitartefakt — sveipedriveren er med.
M44_BEVISROT_FILER = (
    "deploy/staging/m44_fasit.py",
    "deploy/staging/m44-fasit-artefakt.py",
    "deploy/staging/m44-suite-artefakt.py",
    "platform/drift/kampanjesveip.py",
    "platform/drift/kjor_kampanjesveip.py",
)

KRAVGRENSER["m44-fasit-v1"] = {
    # SETTET ER EKSAKT: 14 mottakere, 6 ventede funn (trukket, utløpt 400
    # og 366 — ikke 365 —, over frekvens 3 av 2, og to `ingen_grense`),
    # 77 evidenshendelser — nøyaktig det `m44_fasit.py` bærer, lest av
    # settet i `test_settet_er_fasiten`.
    "mottakere_eksakt": 14,
    "ventede_funn_eksakt": 6,
    "evidenshendelser_eksakt": 77,
    "maks_funnavvik": 0,
    "maks_koeavvik": 0,
    "maks_evidensavvik": 0,
    "krev_sett_sha_lik_innsjekket": True,
...
#: M-26 SERTIFISERING (17/9, natt) — samme lest som M-44/M-17/M-6/M-23.
M26_SUITE_ANDEL: tuple[str, ...] = (
    "platform/core/tests/test_bestilling_tilbud_port.py",
    "platform/core/tests/test_m26_bokforing_port.py",
    "platform/core/tests/test_m26_controller.py",
    "platform/core/tests/test_m26_fasit_port.py",
    "platform/core/tests/test_m26_flate_port.py",
    "platform/core/tests/test_m26_prisbok.py",
    "platform/core/tests/test_m26_sending_port.py",
    "platform/core/tests/test_m26_tilbud_port.py",
    "platform/core/tests/test_m26_tilbudsutloser_port.py",
)

#: M-26 FLIPPEDRILLEN (§0, `rollback-m56-v1`s form): M-26 har én release
#: (`m26-r1`) og en arbeider (`disponit-m26.service`). Blokkert til
#: modulen har en forgjenger.
KRAVGRENSER["m26-rollback-v1"] = {
    "maks_claims_etter_drenering": 0,
    "min_inflight": 1,
    "maks_falske_verdikter": 0,
    "min_rullback_claims": 1,
...
KRAVGRENSER["m26-suite-v1"] = {
    "min_tester": 1500,
    "maks_feilet": 0,
    # Andelens gulv er MÅLT: 71 tester i de åtte eksisterende filene pluss
    # fasitportens fire (17/9). Gulvet står litt under.
    "min_m26_tester": 68,
    "maks_m26_feilet": 0,
    "maks_m26_hoppet": 0,
    "m26_andel_pakrevd": M26_SUITE_ANDEL,
    "punktbinding": {
        "tester_gronne_pa_staging": (
            "maalt.m26_feilet",
            "maalt.m26_tester",
...
M26_SETT_STI = REPOROT / "deploy/staging/m26_fasit.py"

#: Produsentflaten for M-26s fasitartefakt — sveipedriveren er med.
M26_BEVISROT_FILER = (
    "deploy/staging/m26_fasit.py",
    "deploy/staging/m26-fasit-artefakt.py",
    "deploy/staging/m26-suite-artefakt.py",
    "platform/drift/prisboksveip.py",
    "platform/drift/kjor_prisboksveip.py",
)

KRAVGRENSER["m26-fasit-v1"] = {
    # SETTET ER EKSAKT: 12 produkter, 5 ventede funn (utløper om 30 døgn
    # — ikke 31 —, om 5, i dag, og to `ingen_terskel`), 35
    # evidenshendelser — nøyaktig det `m26_fasit.py` bærer, lest av
    # settet i `test_settet_er_fasiten`.
    "produkter_eksakt": 12,
    "ventede_funn_eksakt": 5,
    "evidenshendelser_eksakt": 35,
    "maks_funnavvik": 0,
    "maks_koeavvik": 0,
    "maks_evidensavvik": 0,
    "krev_sett_sha_lik_innsjekket": True,
...
KRAVGRENSER["m44-kampanje-v1"] = {
    ...
    # gjennom modulen (ikke stub-SMTP) — kvitteringen står i basen.
    "krav_ja": ("rundtur_paa_disponit_com",),
    # PUNKTBINDING (#166), lagt til 16/9 — som m23-purring/m17-svar/m6-
    # inntak: bevisrunden 9/9 injiserte feil mot disponit.com, bindingen
    # manglet. De fem som beviser `feilinjisering_til_unntakskø`: policy-
    # grensen stoppet i unntakskøen, ingen levering uten samtykke, ingen
    # dobbel levering, kvitteringen nådde registeret, kill-switch
    # konsumerte ingen par.
    "punktbinding": {
        "feilinjisering_til_unntakskø": (
            "maalt.policygrense_omgaatt_brudd",
            "maalt.levering_uten_samtykke_brudd",
            "maalt.dobbel_levering_samme_oppdrag_brudd",
            "maalt.kvittering_uten_bokforing_brudd",
            "maalt.kill_switch_konsumerte_par_brudd",
        ),
    },
}
...
KRAVGRENSER["m17-svar-v1"] = {
    ...
    "min_forsok": 1,
    "krav_ja": ("rundtur_paa_disponit_com",),
    # PUNKTBINDING (#166), lagt til 16/9 — samme historie som
    # `m23-purring-v1`: bevisrunden 10/9 injiserte ekte feil mot
    # disponit.com og målte hvor de havnet, men uten binding kunne ingen
    # av tallene flippe et punkt.
    #
    # HVORFOR NETTOPP DISSE FEM beviser `feilinjisering_til_unntakskø`:
    #   * `personopplysning_i_svaret` og `okonomisk_lofte_i_svaret` —
    #     et svar policyen ikke slipper ut stoppet i UNNTAKSKØEN som sak,
    #     med henvendelsen intakt. Aldri stille, aldri tillat.
    #   * `svar_uten_mottaker_eller_svarvei` — lukket, i køen, uten
    #     adresse: 409 fra bestillingsveien og `feilet` fra modulen, aldri
    #     en henvendelse som forsvant.
    #   * `kvittering_uten_bokforing` — kvitteringen nådde registeret:
    #     utkastet `sendt`, henvendelsen lukket, evidensen `svar.sendt`.
    #   * `kill_switch_konsumerte_utkast` — bryteren midt i kjeden stoppet
    #     uten å brenne utkastet.
    # Manifestets ord «en feilet klassifisering havner i M-37s kø MED
    # henvendelsen intakt» måles i tillegg av fasitsettet
    # (`m17-fasit-v1`, `maalt.koeavvik`) — to saker til køen, teksten
    # lest tilbake bit for bit.
    "punktbinding": {
        "feilinjisering_til_unntakskø": (
            "maalt.personopplysning_i_svaret_brudd",
            "maalt.okonomisk_lofte_i_svaret_brudd",
            "maalt.svar_uten_mottaker_eller_svarvei_brudd",
            "maalt.kvittering_uten_bokforing_brudd",
            "maalt.kill_switch_konsumerte_utkast_brudd",
        ),
    },
}
...
KRAVGRENSER["m26-tilbud-v1"] = {
    ...
    "krav_ja": ("rundtur_paa_disponit_com",),
    # Sertifiseringen 17/9: manifestets feilinjiseringspunkt bindes til de
    # fem målingene som beviser det (samme lest som m44-kampanje-v1).
    "punktbinding": {
        "feilinjisering_til_unntakskø": (
            "maalt.priser_utenfor_boka_brudd",
            "maalt.klausul_erstattet_brudd",
            "maalt.belop_over_policyens_tak_brudd",
            "maalt.dobbel_sending_samme_oppdrag_brudd",
            "maalt.kill_switch_konsumerte_tilbud_brudd",
        ),
    },
}
...
KRAVGRENSER["m6-inntak-v1"] = {
    ...
    "min_forsok": 1,
    "krav_ja": ("rundtur_paa_disponit_com",),
    # PUNKTBINDING (#166), lagt til 16/9 — samme historie som m23-purring
    # og m17-svar: bevisrunden 10/9 injiserte feil i innhenteren mot
    # disponit.com, men uten binding kunne ingen av tallene flippe et
    # punkt. De fem som beviser `feilinjisering_til_unntakskø`: en
    # autentiseringsfeil gjør kilden `feilet` (ikke stille), en
    # forbigående feil konsumerer ingenting, kill-switch konsumerer
    # ingen kilder, en deaktivert kilde hentes ikke, og et duplikat blir
    # aldri to meldinger.
    "punktbinding": {
        "feilinjisering_til_unntakskø": (
...
ARTEFAKTSKJEMAER: dict[str, str] = {
    ...
    "m17-fasit-v1": "artefakt-m17-fasit-skjema.json",
    "m17-suite-v1": "artefakt-m17-suite-skjema.json",
    "m6-fasit-v1": "artefakt-m6-fasit-skjema.json",
    "m6-suite-v1": "artefakt-m6-suite-skjema.json",
...
def _sjekk_grenser(krav_id: str, art: dict) -> list[str]:
    ...
    if krav_id == "m44-fasit-v1":
        return feil + _grenser_m44_fasit(grense, art)
...
def m17_bevisrot_sha256() -> str:
    """ÉN digest over hele M-17-produsentflaten (samme form som m23)."""
    h = hashlib.sha256()
    for rel in M17_BEVISROT_FILER:
        p = REPOROT / rel
        h.update(rel.encode("utf-8") + b"\x00")
        h.update(hashlib.sha256(p.read_bytes()).digest())
    return h.hexdigest()
...
def _m17_bevisrot_feil(art: dict) -> list[str]:
    oppsett = art.get("oppsett")
    sha = oppsett.get("bevisrot_sha256") if isinstance(oppsett, dict) \
        else None
    if not (isinstance(sha, str) and len(sha) == 64):
...
def _grenser_m17_suite(grense: dict, art: dict) -> list[str]:
    """`m17-suite-v1` — hele suiten på staging, M-17s andel for seg.
    ...
    tall = {}
    for navn in ("tester_totalt", "tester_feilet", "tester_hoppet",
                 "m17_tester", "m17_feilet", "m17_hoppet",
                 "suite_exitkode", "m17_exitkode"):
        verdi, melding = _teller(m, navn, navn)
        if melding:
            feil.append(melding)
        tall[navn] = verdi
    ...
    if tall["tester_feilet"] > grense["maks_feilet"]:
        feil.append(f"tester_feilet={tall['tester_feilet']}, krever <="
                    f" {grense['maks_feilet']}")
    m17_kjorte = tall["m17_tester"] - tall["m17_hoppet"]
    if m17_kjorte < grense["min_m17_tester"]:
        feil.append(f"m17_tester={tall['m17_tester']} minus"
                    f" m17_hoppet={tall['m17_hoppet']} = {m17_kjorte}"
                    f" kjørte, krever >= {grense['min_m17_tester']}")
    if tall["m17_feilet"] > grense["maks_m17_feilet"]:
        feil.append(f"m17_feilet={tall['m17_feilet']}, krever <="
                    f" {grense['maks_m17_feilet']}")
    if tall["m17_hoppet"] > grense["maks_m17_hoppet"]:
        feil.append(f"m17_hoppet={tall['m17_hoppet']}, krever <="
                    f" {grense['maks_m17_hoppet']} — M-17s navngitte"
                    " andel skal være KJØRT, ikke hoppet over")
    if tall["m17_tester"] > tall["tester_totalt"]:
        feil.append(f"m17_tester={tall['m17_tester']} >"
                    f" tester_totalt={tall['tester_totalt']} — andelen"
                    " kan ikke overstige helheten")
    for navn in ("suite_exitkode", "m17_exitkode"):
        if tall[navn] != 0:
            feil.append(f"{navn}={tall[navn]} — pytest avsluttet unormalt;"
                        " en junit-XML fra en avbrutt kjøring teller bare"
                        " testene som rakk å bli ferdige")
    oppsett = art.get("oppsett")
    feil += _m17_bevisrot_feil(art)
    filer = oppsett.get("m17_filer") if isinstance(oppsett, dict) else None
    if not (isinstance(filer, list) and filer
            and all(isinstance(x, str) and x for x in filer)):
        feil.append("oppsett.m17_filer mangler — M-17s andel skal være"
                    " NAVNGITT, ikke antatt (delingsbetingelsen)")
    elif sorted(filer) != sorted(grense["m17_andel_pakrevd"]):
        mangler = sorted(set(grense["m17_andel_pakrevd"]) - set(filer))
        ekstra = sorted(set(filer) - set(grense["m17_andel_pakrevd"]))
        feil.append(
            "oppsett.m17_filer er ikke det godkjente utvalget"
            + (f"; mangler {mangler}" if mangler else "")
            + (f"; ukjente {ekstra}" if ekstra else "")
...
M17_AKSER = ("funnavvik", "klassifiseringsavvik", "koeavvik", "evidensavvik")


def _grenser_m17_fasit(grense: dict, art: dict) -> list[str]:
    """`m17-fasit-v1` — henvendelsessettet mot KJENT fasit. Dommen
    ...
    if not (isinstance(sha, str) and len(sha) == 64):
        ...
        try:
            lokal = hashlib.sha256(M17_SETT_STI.read_bytes()).hexdigest()
        except OSError as e:
            feil.append(f"settdriveren lot seg ikke hashe lokalt: {e}")
        else:
            if sha != lokal:
    ...
    if not (isinstance(tenanter, list) and len(tenanter) >= 2):
        feil.append("oppsett.tenanter har færre enn to — uten den"
                    " regelløse tenanten er det umålt at regelen er"
                    " grunnen til klassifiseringen")
    avvik = art.get("avvik")
    if not isinstance(avvik, dict) or not avvik:
        return feil + ["artefaktet mangler `avvik` — en dom uten listene"
                       " sine kan ikke re-regnes"]
    talt = {akse: 0 for akse in M17_AKSER}
    for rolle, d in avvik.items():
        if not isinstance(d, dict):
            return feil + [f"avvik[{rolle!r}] er ikke et oppslag"]
        for nokkel in talt:
    ...
    for nokkel in M17_AKSER:
        ...
        if talt[nokkel] > tak:
            feil.append(f"{talt[nokkel]} {nokkel} re-talt av listene,"
                        f" taket er {tak}")
    for nokkel, fasit in (
            ("henvendelser", grense["henvendelser_eksakt"]),
            ("ventede_funn", grense["ventede_funn_eksakt"]),
            ("regelklassifisert", grense["regelklassifisert_eksakt"]),
            ("evidenshendelser", grense["evidenshendelser_eksakt"])):
        verdi, melding = _teller(m, nokkel, nokkel)
        if melding:
            feil.append(melding)
...
def m44_bevisrot_sha256() -> str:
    """ÉN digest over hele M-44-produsentflaten (samme form som m23)."""
    h = hashlib.sha256()
    for rel in M44_BEVISROT_FILER:
        p = REPOROT / rel
        h.update(rel.encode("utf-8") + b"\x00")
        h.update(hashlib.sha256(p.read_bytes()).digest())
    return h.hexdigest()
...
def _m44_bevisrot_feil(art: dict) -> list[str]:
    oppsett = art.get("oppsett")
    sha = oppsett.get("bevisrot_sha256") if isinstance(oppsett, dict) \
        else None
    if not (isinstance(sha, str) and len(sha) == 64):
...
def _grenser_m44_suite(grense: dict, art: dict) -> list[str]:
    """`m44-suite-v1` — hele suiten på staging, M-44s andel for seg.
    Formen er `_grenser_m23_suite` sin, av de samme grunnene."""
    feil: list[str] = []
    m = art.get("maalt")
    if not isinstance(m, dict):
    ...
    tall = {}
    for navn in ("tester_totalt", "tester_feilet", "tester_hoppet",
                 "m44_tester", "m44_feilet", "m44_hoppet",
                 "suite_exitkode", "m44_exitkode"):
        verdi, melding = _teller(m, navn, navn)
        if melding:
            feil.append(melding)
        tall[navn] = verdi
    ...
    if tall["tester_feilet"] > grense["maks_feilet"]:
        feil.append(f"tester_feilet={tall['tester_feilet']}, krever <="
                    f" {grense['maks_feilet']}")
    m44_kjorte = tall["m44_tester"] - tall["m44_hoppet"]
    if m44_kjorte < grense["min_m44_tester"]:
        feil.append(f"m44_tester={tall['m44_tester']} minus"
                    f" m44_hoppet={tall['m44_hoppet']} = {m44_kjorte}"
                    f" kjørte, krever >= {grense['min_m44_tester']}")
    if tall["m44_feilet"] > grense["maks_m44_feilet"]:
        feil.append(f"m44_feilet={tall['m44_feilet']}, krever <="
                    f" {grense['maks_m44_feilet']}")
    if tall["m44_hoppet"] > grense["maks_m44_hoppet"]:
        feil.append(f"m44_hoppet={tall['m44_hoppet']}, krever <="
                    f" {grense['maks_m44_hoppet']} — M-44s navngitte"
                    " andel skal være KJØRT, ikke hoppet over")
    if tall["m44_tester"] > tall["tester_totalt"]:
        feil.append(f"m44_tester={tall['m44_tester']} >"
                    f" tester_totalt={tall['tester_totalt']} — andelen"
                    " kan ikke overstige helheten")
    for navn in ("suite_exitkode", "m44_exitkode"):
        if tall[navn] != 0:
            feil.append(f"{navn}={tall[navn]} — pytest avsluttet unormalt;"
                        " en junit-XML fra en avbrutt kjøring teller bare"
                        " testene som rakk å bli ferdige")
    oppsett = art.get("oppsett")
    feil += _m44_bevisrot_feil(art)
    filer = oppsett.get("m44_filer") if isinstance(oppsett, dict) else None
    if not (isinstance(filer, list) and filer
            and all(isinstance(x, str) and x for x in filer)):
        feil.append("oppsett.m44_filer mangler — M-44s andel skal være"
                    " NAVNGITT, ikke antatt (delingsbetingelsen)")
    elif sorted(filer) != sorted(grense["m44_andel_pakrevd"]):
        mangler = sorted(set(grense["m44_andel_pakrevd"]) - set(filer))
        ekstra = sorted(set(filer) - set(grense["m44_andel_pakrevd"]))
        feil.append(
            "oppsett.m44_filer er ikke det godkjente utvalget"
            + (f"; mangler {mangler}" if mangler else "")
            + (f"; ukjente {ekstra}" if ekstra else "")
            + " — delingsbetingelsen krever de PINNEDE målingene")
    return feil
...
M44_AKSER = ("funnavvik", "koeavvik", "evidensavvik")


def _grenser_m44_fasit(grense: dict, art: dict) -> list[str]:
    """`m44-fasit-v1` — henvendelsessettet mot KJENT fasit. Dommen
    RE-REGNES av avvikslistene artefaktet selv bærer (som m23)."""
    feil: list[str] = []
    m = art.get("maalt")
    if not isinstance(m, dict):
    ...
    if not (isinstance(sha, str) and len(sha) == 64):
        ...
        try:
            lokal = hashlib.sha256(M44_SETT_STI.read_bytes()).hexdigest()
        except OSError as e:
            feil.append(f"settdriveren lot seg ikke hashe lokalt: {e}")
        else:
            if sha != lokal:
    ...
    if not (isinstance(tenanter, list) and len(tenanter) >= 2):
        feil.append("oppsett.tenanter har færre enn to — uten den"
                    " grenseløse tenanten er `ingen_grense` umålt")
    avvik = art.get("avvik")
    if not isinstance(avvik, dict) or not avvik:
        return feil + ["artefaktet mangler `avvik` — en dom uten listene"
                       " sine kan ikke re-regnes"]
    talt = {akse: 0 for akse in M44_AKSER}
    for rolle, d in avvik.items():
        if not isinstance(d, dict):
            return feil + [f"avvik[{rolle!r}] er ikke et oppslag"]
        for nokkel in talt:
    ...
    for nokkel in M44_AKSER:
        ...
        if talt[nokkel] > tak:
            feil.append(f"{talt[nokkel]} {nokkel} re-talt av listene,"
                        f" taket er {tak}")
    for nokkel, fasit in (
            ("mottakere", grense["mottakere_eksakt"]),
            ("ventede_funn", grense["ventede_funn_eksakt"]),
            ("evidenshendelser", grense["evidenshendelser_eksakt"])):
        verdi, melding = _teller(m, nokkel, nokkel)
        if melding:
...
def m26_bevisrot_sha256() -> str:
    """ÉN digest over hele M-26-produsentflaten (samme form som m23)."""
    h = hashlib.sha256()
    for rel in M26_BEVISROT_FILER:
        p = REPOROT / rel
        h.update(rel.encode("utf-8") + b"\x00")
        h.update(hashlib.sha256(p.read_bytes()).digest())
    return h.hexdigest()
...
def _m26_bevisrot_feil(art: dict) -> list[str]:
    oppsett = art.get("oppsett")
    sha = oppsett.get("bevisrot_sha256") if isinstance(oppsett, dict) \
        else None
    if not (isinstance(sha, str) and len(sha) == 64):
...
def _grenser_m26_suite(grense: dict, art: dict) -> list[str]:
    """`m26-suite-v1` — hele suiten på staging, M-26s andel for seg.
    Formen er `_grenser_m23_suite` sin, av de samme grunnene."""
    feil: list[str] = []
    m = art.get("maalt")
    if not isinstance(m, dict):
    ...
    tall = {}
    for navn in ("tester_totalt", "tester_feilet", "tester_hoppet",
                 "m26_tester", "m26_feilet", "m26_hoppet",
                 "suite_exitkode", "m26_exitkode"):
        verdi, melding = _teller(m, navn, navn)
        if melding:
            feil.append(melding)
        tall[navn] = verdi
    ...
    if tall["tester_feilet"] > grense["maks_feilet"]:
        feil.append(f"tester_feilet={tall['tester_feilet']}, krever <="
                    f" {grense['maks_feilet']}")
    m26_kjorte = tall["m26_tester"] - tall["m26_hoppet"]
    if m26_kjorte < grense["min_m26_tester"]:
        feil.append(f"m26_tester={tall['m26_tester']} minus"
                    f" m26_hoppet={tall['m26_hoppet']} = {m26_kjorte}"
                    f" kjørte, krever >= {grense['min_m26_tester']}")
    if tall["m26_feilet"] > grense["maks_m26_feilet"]:
        feil.append(f"m26_feilet={tall['m26_feilet']}, krever <="
                    f" {grense['maks_m26_feilet']}")
    if tall["m26_hoppet"] > grense["maks_m26_hoppet"]:
        feil.append(f"m26_hoppet={tall['m26_hoppet']}, krever <="
                    f" {grense['maks_m26_hoppet']} — M-26s navngitte"
                    " andel skal være KJØRT, ikke hoppet over")
    if tall["m26_tester"] > tall["tester_totalt"]:
        feil.append(f"m26_tester={tall['m26_tester']} >"
                    f" tester_totalt={tall['tester_totalt']} — andelen"
                    " kan ikke overstige helheten")
    for navn in ("suite_exitkode", "m26_exitkode"):
        if tall[navn] != 0:
            feil.append(f"{navn}={tall[navn]} — pytest avsluttet unormalt;"
                        " en junit-XML fra en avbrutt kjøring teller bare"
                        " testene som rakk å bli ferdige")
    oppsett = art.get("oppsett")
    feil += _m26_bevisrot_feil(art)
    filer = oppsett.get("m26_filer") if isinstance(oppsett, dict) else None
    if not (isinstance(filer, list) and filer
            and all(isinstance(x, str) and x for x in filer)):
        feil.append("oppsett.m26_filer mangler — M-26s andel skal være"
                    " NAVNGITT, ikke antatt (delingsbetingelsen)")
    elif sorted(filer) != sorted(grense["m26_andel_pakrevd"]):
        mangler = sorted(set(grense["m26_andel_pakrevd"]) - set(filer))
        ekstra = sorted(set(filer) - set(grense["m26_andel_pakrevd"]))
        feil.append(
            "oppsett.m26_filer er ikke det godkjente utvalget"
            + (f"; mangler {mangler}" if mangler else "")
            + (f"; ukjente {ekstra}" if ekstra else "")
            + " — delingsbetingelsen krever de PINNEDE målingene")
    return feil
...
M26_AKSER = ("funnavvik", "koeavvik", "evidensavvik")


def _grenser_m26_fasit(grense: dict, art: dict) -> list[str]:
    """`m26-fasit-v1` — prisboksettet mot KJENT fasit. Dommen
    RE-REGNES av avvikslistene artefaktet selv bærer (som m23)."""
    feil: list[str] = []
    m = art.get("maalt")
    if not isinstance(m, dict):
    ...
    if not (isinstance(sha, str) and len(sha) == 64):
        ...
        try:
            lokal = hashlib.sha256(M26_SETT_STI.read_bytes()).hexdigest()
        except OSError as e:
            feil.append(f"settdriveren lot seg ikke hashe lokalt: {e}")
        else:
            if sha != lokal:
    ...
    if not isinstance(avvik, dict) or not avvik:
        return feil + ["artefaktet mangler `avvik` — en dom uten listene"
                       " sine kan ikke re-regnes"]
    talt = {akse: 0 for akse in M26_AKSER}
    for rolle, d in avvik.items():
        if not isinstance(d, dict):
            return feil + [f"avvik[{rolle!r}] er ikke et oppslag"]
        for nokkel in talt:
...
def m6_bevisrot_sha256() -> str:
    """ÉN digest over hele M-6-produsentflaten (samme form som m23)."""
    h = hashlib.sha256()
    for rel in M6_BEVISROT_FILER:
        p = REPOROT / rel
        h.update(rel.encode("utf-8") + b"\x00")
        h.update(hashlib.sha256(p.read_bytes()).digest())
    return h.hexdigest()
...
def _m6_bevisrot_feil(art: dict) -> list[str]:
    oppsett = art.get("oppsett")
    sha = oppsett.get("bevisrot_sha256") if isinstance(oppsett, dict) \
        else None
    if not (isinstance(sha, str) and len(sha) == 64):
...
def _grenser_m6_suite(grense: dict, art: dict) -> list[str]:
    """`m6-suite-v1` — hele suiten på staging, M-6s andel for seg.
    Formen er `_grenser_m23_suite` sin, av de samme grunnene."""
    feil: list[str] = []
    m = art.get("maalt")
    if not isinstance(m, dict):
    ...
    tall = {}
    for navn in ("tester_totalt", "tester_feilet", "tester_hoppet",
                 "m6_tester", "m6_feilet", "m6_hoppet",
                 "suite_exitkode", "m6_exitkode"):
        verdi, melding = _teller(m, navn, navn)
        if melding:
            feil.append(melding)
        tall[navn] = verdi
    ...
    if tall["tester_feilet"] > grense["maks_feilet"]:
        feil.append(f"tester_feilet={tall['tester_feilet']}, krever <="
                    f" {grense['maks_feilet']}")
    m6_kjorte = tall["m6_tester"] - tall["m6_hoppet"]
    if m6_kjorte < grense["min_m6_tester"]:
        feil.append(f"m6_tester={tall['m6_tester']} minus"
                    f" m6_hoppet={tall['m6_hoppet']} = {m6_kjorte}"
                    f" kjørte, krever >= {grense['min_m6_tester']}")
    if tall["m6_feilet"] > grense["maks_m6_feilet"]:
        feil.append(f"m6_feilet={tall['m6_feilet']}, krever <="
                    f" {grense['maks_m6_feilet']}")
    if tall["m6_hoppet"] > grense["maks_m6_hoppet"]:
        feil.append(f"m6_hoppet={tall['m6_hoppet']}, krever <="
                    f" {grense['maks_m6_hoppet']} — M-6s navngitte"
                    " andel skal være KJØRT, ikke hoppet over")
    if tall["m6_tester"] > tall["tester_totalt"]:
        feil.append(f"m6_tester={tall['m6_tester']} >"
                    f" tester_totalt={tall['tester_totalt']} — andelen"
                    " kan ikke overstige helheten")
    for navn in ("suite_exitkode", "m6_exitkode"):
        if tall[navn] != 0:
            feil.append(f"{navn}={tall[navn]} — pytest avsluttet unormalt;"
                        " en junit-XML fra en avbrutt kjøring teller bare"
                        " testene som rakk å bli ferdige")
    oppsett = art.get("oppsett")
    feil += _m6_bevisrot_feil(art)
    filer = oppsett.get("m6_filer") if isinstance(oppsett, dict) else None
    if not (isinstance(filer, list) and filer
            and all(isinstance(x, str) and x for x in filer)):
        feil.append("oppsett.m6_filer mangler — M-6s andel skal være"
                    " NAVNGITT, ikke antatt (delingsbetingelsen)")
    elif sorted(filer) != sorted(grense["m6_andel_pakrevd"]):
        mangler = sorted(set(grense["m6_andel_pakrevd"]) - set(filer))
        ekstra = sorted(set(filer) - set(grense["m6_andel_pakrevd"]))
        feil.append(
            "oppsett.m6_filer er ikke det godkjente utvalget"
            + (f"; mangler {mangler}" if mangler else "")
            + (f"; ukjente {ekstra}" if ekstra else "")
            + " — delingsbetingelsen krever de PINNEDE målingene")
    return feil
...
M6_AKSER = ("inntaksavvik", "broavvik", "svaravvik", "evidensavvik")


def _grenser_m6_fasit(grense: dict, art: dict) -> list[str]:
    """`m6-fasit-v1` — henvendelsessettet mot KJENT fasit. Dommen
    RE-REGNES av avvikslistene artefaktet selv bærer (som m23)."""
    feil: list[str] = []
    m = art.get("maalt")
    if not isinstance(m, dict):
    ...
    if not (isinstance(sha, str) and len(sha) == 64):
        ...
        try:
            lokal = hashlib.sha256(M6_SETT_STI.read_bytes()).hexdigest()
        except OSError as e:
            feil.append(f"settdriveren lot seg ikke hashe lokalt: {e}")
        else:
            if sha != lokal:
    ...
    if not (isinstance(tenanter, list) and len(tenanter) >= 2):
        feil.append("oppsett.tenanter har færre enn to — uten den"
                    " scopeløse tenanten er det umålt at samtykket stenger"
                    " svarveien")
    avvik = art.get("avvik")
    if not isinstance(avvik, dict) or not avvik:
        return feil + ["artefaktet mangler `avvik` — en dom uten listene"
                       " sine kan ikke re-regnes"]
    talt = {akse: 0 for akse in M6_AKSER}
    for rolle, d in avvik.items():
        if not isinstance(d, dict):
            return feil + [f"avvik[{rolle!r}] er ikke et oppslag"]
        for nokkel in talt:
    ...
    for nokkel in M6_AKSER:
        ...
        if talt[nokkel] > tak:
            feil.append(f"{talt[nokkel]} {nokkel} re-talt av listene,"
                        f" taket er {tak}")
    for nokkel, fasit in (
            ("meldinger", grense["meldinger_eksakt"]),
            ("nye_runde1", grense["nye_runde1_eksakt"]),
            ("nye_runde2", grense["nye_runde2_eksakt"]),
            ("henvendelser", grense["henvendelser_eksakt"]),
            ("sendt", grense["sendt_eksakt"]),
            ("evidenshendelser", grense["evidenshendelser_eksakt"])):
        verdi, melding = _teller(m, nokkel, nokkel)
        if melding:
            feil.append(melding)
            ...
            feil.append(f"{nokkel}={verdi}, fasiten er {fasit} — eksakt,"
                        " aldri «nesten»")
    ms, melding = _teller(m, "innhentingstid_ms", "innhentingstid_ms")
    if melding:
        feil.append(melding)
    elif ms < 1:
        feil.append("innhentingstid_ms=0 — et inntak ingen tok tiden på er"
                    " ikke en ytelsesmåling, og 0 består ethvert tak")
    elif ms > grense["maks_innhentingstid_ms"]:
        feil.append(f"innhentingstid_ms={ms}, taket er"
                    f" {grense['maks_innhentingstid_ms']}")
    else:
        antall, _ = _teller(m, "meldinger", "meldinger")
        if antall and ms / antall > grense["maks_innhentingstid_ms_per_melding"]:
            feil.append(f"innhentingstid_ms={ms} over {antall} meldinger er"
                        f" {ms / antall:.0f} ms per melding, taket er"
                        f" {grense['maks_innhentingstid_ms_per_melding']}")
    return feil
