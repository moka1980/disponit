-- 084 — M-16 nøkkeltall fase 2 (PR-A): definer-settet bygges på nytt.
--
-- 084 overtar som M-16s definer-migrasjon. Ryddeblokken spør katalogen
-- om alle `m16_%`-definere — fra 051 eller en tidligere kjøring av
-- DENNE fila — og bygger HELE settet på nytt: ni definere.
--   * Seks kopiert ordrett fra 051: m16_beslutninger, m16_aktiveringer,
--     m16_oppdrag, m16_unntak_aktivitet, m16_unntak_apne, m16_tick.
--   * m16_frekvens (NY) erstatter m16_frekvensreservasjoner-skalaren:
--     samme kilde og samme vindu, men som partisjon per handling med
--     GROUPING SETS — rå tellinger ved siden av totalen, samme skann.
--     Ingen overgangsperiode: flaten er skalarens eneste konsument.
--   * m16_unntak_lukkede får kolonnen varighet_sum_s — summen av de
--     radvise varighetene over HELE vinduet, i samme vindusform som
--     antall_totalt (etter WHERE, før ORDER BY og LIMIT: samme skann
--     som radene, upåvirket av visningstaket).
--   * m16_tick_alltid (NY): tellingen over all tid, uten vindu — så
--     flaten kan skille «tomt vindu» fra «ingen kjøring registrert».
--
-- Grensen (M-16 §1) står: hvert avledet tall skal kunne leses tilbake
-- til rå tellinger som står VED SIDEN AV i samme svar, fra samme skann.
-- Derfor finnes det INGEN divisjon i denne fila: andeler og snitt
-- regnes i API-laget, av teller og nevner definerne leverer sammen.
-- Sum og count er de eneste aggregatene; median leveres ikke.
--
-- Formen er 051-form (SPEIL): STABLE SECURITY DEFINER eid av
-- disponit_m37_claimer, search_path pg_catalog, krev_tenantkontekst
-- (SP-1) + `tenant = p_tenant` i hvert skann. Flaten leser aldri
-- tabeller direkte (SP-7) — runtime får KUN EXECUTE. Suminvarianten
-- per partisjon bæres av GROUPING SETS ((nokkel), ()) som i 051, og
-- totalraden sier selv at den er totalen (`er_total` — en egenskap ved
-- raden, aldri en reservert nøkkelverdi).
...
-- `m16_*`-definere som faktisk finnes — fra 051, som 084 overtar for,
-- eller fra en tidligere kjøring av DENNE fila; prefikset er hele
-- settet — og de fjernes uten at noen signatur skrives ned et andre
-- sted.
DO $rydd$
...
-- Frekvensreservasjoner: partisjon per handling + total, samme skann —
-- aksen frekvens_oppslagsindeks allerede bærer. Erstatter 051-skalaren
-- m16_frekvensreservasjoner: kortet fikk ett tall uten deler; nå står
-- delene ved siden av totalen, talt i samme skann (grensen §1).
-- `handling` er NOT NULL i kilden; coalesce står likevel, i samme form
-- som de andre kortene — en framtidig NULL havner synlig i «ukjent»,
-- i egen gruppe OG i totalen, aldri stille utenfor.
CREATE OR REPLACE FUNCTION m16_frekvens(
    p_tenant TEXT, p_fra TIMESTAMPTZ, p_til TIMESTAMPTZ)
RETURNS TABLE(er_total BOOLEAN, nokkel TEXT, antall BIGINT)
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = pg_catalog AS $$
BEGIN
    PERFORM public.krev_tenantkontekst(p_tenant, 'm16_frekvens');
    RETURN QUERY
    SELECT GROUPING(f.handling) = 1,
           CASE WHEN GROUPING(f.handling) = 1 THEN NULL
                ELSE coalesce(f.handling, 'ukjent') END,
           count(*)
      FROM public.frekvens_hendelser f
     WHERE f.tenant = p_tenant
       AND f.tidspunkt >= p_fra AND f.tidspunkt < p_til
     GROUP BY GROUPING SETS ((f.handling), ());
END $$;
...
--
-- `varighet_sum_s` er summen av de radvise varighetene over HELE
-- vinduet, i NØYAKTIG samme vindusform som `antall_totalt`: etter
-- WHERE, før ORDER BY og LIMIT — samme skann, hele settet, upåvirket
-- av visningstaket. Teller (sum) og nevner (antall) for snittet
-- API-laget regner står dermed VED SIDEN AV hverandre i samme svar
-- (grensen §1); divisjonen skjer aldri her.
...
RETURNS TABLE(id BIGINT, kategori TEXT, sakstype TEXT, status TEXT,
              ...
              antall_totalt BIGINT, varighet_sum_s BIGINT)
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = pg_catalog AS $$
...
BEGIN
    ...
    SELECT u.id, coalesce(u.kategori, 'ukjent'), u.sakstype, u.status,
           ...
           count(*) OVER (),
           (sum(EXTRACT(EPOCH FROM (u.status_ts - u.ts))) OVER ())::bigint
      FROM public.unntak u
...
-- Planer, ALL TID: tellingen uten vindu — flaten kan da skille «tomt
-- vindu» fra «ingen kjøring registrert i det hele tatt» uten å påstå
-- et fravær som aldri er målt. Tilstandsform som m16_unntak_apne:
-- ingen vindusparametre, upåvirket av vindusvelgeren.
CREATE OR REPLACE FUNCTION m16_tick_alltid(p_tenant TEXT)
RETURNS BIGINT
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = pg_catalog AS $$
DECLARE v BIGINT;
BEGIN
    PERFORM public.krev_tenantkontekst(p_tenant, 'm16_tick_alltid');
    SELECT count(*) INTO v FROM public.bestillingsplan_tick t
     WHERE t.tenant = p_tenant;
    RETURN v;
END $$;
...
-- i takt.
--
-- Navnene er intensjonen (ni definere, verken flere eller færre);
-- signaturene henter vi fra funksjonene CREATE-setningene over
-- FAKTISK laget. Avviket kan da bare gå én vei, og `antall <> 9`
...
BEGIN
        ...
        SELECT p.oid::regprocedure::text
         ...
         WHERE n.nspname = 'public'
           AND p.proname = ANY (ARRAY[
                   'm16_beslutninger', 'm16_frekvens',
                   'm16_aktiveringer', 'm16_oppdrag',
                   'm16_unntak_aktivitet', 'm16_unntak_lukkede',
                   'm16_unntak_apne', 'm16_tick', 'm16_tick_alltid'])
         ORDER BY 1
    LOOP
    ...
    IF antall <> 9 THEN
        RAISE EXCEPTION
            'M-16 (084): fant % m16-definere, forventet 9 — en definer '
