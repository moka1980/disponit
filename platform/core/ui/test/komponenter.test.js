...
test("Faner: ARIA-mønsteret, ikke bare knapper som ser ut som faner", async () => {
  ...
  const panel = rot.querySelector('[role="tabpanel"]:not([hidden])');
  assert.equal(panel.textContent, "innhold A");
  // Ett panel per fane, og NØYAKTIG ett synlig. De inaktive må finnes i
  // DOM-en, ellers peker fanenes `aria-controls` på ID-er som ikke er der.
  assert.equal(rot.querySelectorAll('[role="tabpanel"]').length, 3);
  assert.equal(rot.querySelectorAll('[role="tabpanel"]:not([hidden])').length, 1);
  for (const f of faner) {
    const id = f.getAttribute("aria-controls");
    assert.ok(rot.querySelector(`#${id}`),
      `fanen «${f.textContent}» peker på panelet ${id}, som ikke finnes`);
  }
