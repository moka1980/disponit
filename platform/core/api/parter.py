...
# CSRF + AKTØR KOMMER FRA HUSETS EGEN HJELPER, `_browserkontekst`
# (policyadmin_http), som syv andre skriveveier alt bruker. Første utgave
# skrev sin egen kopi av dobbel-innsendingen, og den var riktig — men den
# satte AKTØREN til `auth.aktor`, altså `token:sesjon:bid_…`. Eier la inn
# den første kunden og revisjonssporet sa en økt-id der det skulle stått
# et menneske.
#
# `_browserkontekst` gjør tre ting i én: autentiserer mot scopet,
# håndhever dobbel-innsendingen for browsersesjoner (og hopper over den
# for maskinveien, der en header ikke kan sendes av en fremmed side), og
# setter `disponit.tenant/aktor/request_id` med BRUKER-ID-en som aktør.
# Den reiser `_Avbrudd` med ferdig feilsvar; `_med_conn` fanger det.
        ...
        try:
            with conn.transaction():
                pid = conn.execute(
                    ...
                    (tenant, ref.strip(), navn.strip(), org, parttype, aktor)).fetchone()[0]
        ...
        try:
            with conn.transaction():
                psn = conn.execute("SELECT tenant_pseudonym(%s,%s)",
                                   (tenant, verdi.strip())).fetchone()[0]
                kid = conn.execute(
                    "SELECT part_sett_kontakt(%s,%s,%s,%s,%s,%s,%s,%s,%s,"
                    "%s,%s)",
                    (tenant, pid, kanal, _maske(verdi, kanal), ct,
                     nonce, key_id, psn, bool(k.get("primar", True)),
                     (merkelapp or None), aktor)).fetchone()[0]
