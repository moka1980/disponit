#!/usr/bin/env python3
"""Registrer m44_kampanje-kjeden gjennom de HERDEDE funksjonene
...
Kjøres på verten når kampanjemodulen skal i drift (ARC B kampanje, PR 4):

    DISPONIT_MIGRATOR_URL=… python3 deploy/staging/registrer-m44-kampanje.py \\
        <release_id> <kontrakt_hash> <artifact_digest> \\
        <payload_skjema_hash> <kvittering_skjema_hash>

Kontraktklassen er `krever_outbox`/`kompenserende`: en kampanje-e-post
er levert når den er levert, og policyens reversering er
`kampanje.send_korreksjon` — en ny e-post, ikke en angring.
...
Ingen artefakttype: kampanjen produserer ingen rapport, kvitteringen ER
...
MANIFEST = Path("platform/modules/m44_kampanje/manifest.yaml")
