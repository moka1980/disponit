#!/usr/bin/env bash
# M-14 bokføring (ARC B) — vertsoppsett i ett (M-17-formen, ARC B bokføring PR 5). Kjør som root:
#   sudo bash deploy/staging/m14-oppsett.sh
# INGEN NY HEMMELIGHET: modulen kvitterer som `v_regnskap`, og den nøkkelen
...
# 0640-fil — aldri via argument eller samtale. Ingen SMTP: modulen sender
# ingenting; bokføringen skjer når kvitteringen når registeret (167).
...
MODUL=m14_fakturakontroll; REL=m14-r1; MILJO=staging; AKTOR=m14-utrulling
[ "$(id -u)" = 0 ] || { echo "kjør med sudo"; exit 1; }
cd "$AKTIV"

echo "== 1/5 v_regnskap i DISPONIT_ATT_NOKLER (finnes siden M-1)"
for fil in /etc/disponit/api/DISPONIT_ATT_NOKLER /etc/disponit/plan/DISPONIT_ATT_NOKLER; do
  python3 -c "import json,sys; d=json.load(open('$fil')); k=d.get('v_regnskap') or {}; sys.exit(0 if k else 1)" \
    || { echo "   $fil mangler v_regnskap — kjør opp.sh først"; exit 1; }
  echo "   $fil har v_regnskap"
done
NOKKEL_ID=$(python3 -c "import json; d=json.load(open('/etc/disponit/api/DISPONIT_ATT_NOKLER')); print(sorted(d['v_regnskap'])[0])")

echo "== 2/5 konto, konfig, kvitteringsnøkkel, unit"
id disponit-m14 >/dev/null 2>&1 || useradd --system --home /nonexistent --shell /usr/sbin/nologin disponit-m14
install -d -m 0750 -o root -g disponit-m14 /etc/disponit/m14
{ echo "DISPONIT_API_URL=https://disponit.com"
  echo "DISPONIT_KVITTERINGSNOKKEL=/etc/disponit/m14/kvitteringsnokkel.json"
} > /etc/disponit/m14/konfig
# Nøkkelen kopieres fil-til-fil i python (aldri som argument, aldri i ps).
umask 077
python3 - "$NOKKEL_ID" <<'PY' > /etc/disponit/m14/kvitteringsnokkel.json
import json, sys
d = json.load(open("/etc/disponit/api/DISPONIT_ATT_NOKLER"))
nid = sys.argv[1]
print(json.dumps({"verifikator": "v_regnskap", "nokkel_id": nid,
                  "hemmelighet": d["v_regnskap"][nid]}))
PY
umask 022
chown root:disponit-m14 /etc/disponit/m14/konfig /etc/disponit/m14/kvitteringsnokkel.json
chmod 0640 /etc/disponit/m14/konfig /etc/disponit/m14/kvitteringsnokkel.json
cp deploy/staging/disponit-m14.service /etc/systemd/system/disponit-m14.service
systemctl daemon-reload
# Tjenesten leser kvitteringsnøkkelen selv, og /etc/disponit er lukket for andre:
# gjennomgang (--x) for gruppen, som for m57.
setfacl -m g:disponit-m14:--x /etc/disponit
echo "   kvitteringsnøkkel: v_regnskap/$NOKKEL_ID"

echo "== 3/5 registrering av modulkjeden"
KONTRAKT=$(sha256sum platform/core/oppdragskontrakt.py | cut -c1-64)
KVITT=$(sha256sum platform/modules/m14_fakturakontroll/controller.py | cut -c1-64)
PAYLOAD=$(PYTHONPATH=platform/core "$PY" -c "import json,hashlib,oppdragskontrakt as o; print(hashlib.sha256(json.dumps(sorted(o.OPPDRAGSTYPER['faktura.bokfor'].felter)).encode()).hexdigest())")
DIGEST=$(tar -C platform/modules -cf - m14_fakturakontroll | sha256sum | cut -c1-64)
echo "$KONTRAKT" > /etc/disponit/m14/kontrakt-hash
set -a; . "$ENVFIL"; set +a
if sudo -u postgres psql -d disponit -Atc "SELECT 1 FROM modulrelease WHERE modul_id='$MODUL' AND release_id='$REL'" | grep -q 1; then
  ...
  "$PY" deploy/staging/registrer-m14-fakturakontroll.py "$REL" "$KONTRAKT" "$DIGEST" "$PAYLOAD" "$KVITT" | sed 's/^/   /'
fi
...
if [ -s /etc/disponit/m14/DISPONIT_MODULTOKEN ]; then
  ...
  printf '%s' "$TOK" > /etc/disponit/m14/DISPONIT_MODULTOKEN
  chown root:disponit-m14 /etc/disponit/m14/DISPONIT_MODULTOKEN; chmod 0640 /etc/disponit/m14/DISPONIT_MODULTOKEN
...
systemctl enable --now disponit-m14 >/dev/null 2>&1; systemctl restart disponit-m14
sleep 6
journalctl -u disponit-m14 -n 5 --no-pager | sed 's/^/   /'
echo "== ferdig"
