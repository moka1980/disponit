#!/usr/bin/env python3
"""Registrer m14_fakturakontroll-kjeden gjennom de HERDEDE funksjonene
...
Kjøres på verten når bokføringsmodulen skal i drift (ARC B bokføring, PR 3):

    DISPONIT_MIGRATOR_URL=… python3 deploy/staging/registrer-m14-fakturakontroll.py \\
        <release_id> <kontrakt_hash> <artifact_digest> \\
        <payload_skjema_hash> <kvittering_skjema_hash>

Kontraktklassen er `krever_outbox`/`kompenserende`: et bilag som er
registrert reverseres med et motbilag, ikke ved sletting (101s vakt
nekter DELETE) — bransjemalens reversering for `faktura.bokfor` er
`kompenserende`.
...
def main() -> int:
    ...
    with psycopg.connect(dsn) as c:
        ...
        for ot in OPPDRAGSTYPER:
            c.execute("SELECT registrer_oppdragstype(%s, %s, 1, %s,"
                      " 'deploy')", (ot, MODUL, kontrakt_hash))
        c.execute("RESET ROLE")
        c.commit()
    print(f"registrert: {MODUL} {release_id} — {', '.join(OPPDRAGSTYPER)}"
