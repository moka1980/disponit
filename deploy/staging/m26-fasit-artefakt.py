#!/usr/bin/env python3
"""Staging-leddet for M-26s fasitartefakt (sertifiseringen 17/9, natt).

Driver NØYAKTIG samme sett som CI-leddet (`m26_fasit.kjor_sett`) gjennom
de EKTE dørene på verten — kundens dører som runtime, prisboksveipen som SVEIPEROLLEN, og
flaten lest gjennom `api.prisbok.svar_for` — og skriver artefaktet fra
avvikene målingen fant.

BRUK (på verten, som root, med basen oppe):
    /opt/disponit/.venv/bin/python deploy/staging/m26-fasit-artefakt.py \
        [--vert https://disponit.com] [--ut …]

RYDDER IKKE ETTER SEG (som de andre): tenantnavnene bærer runden.
Fasit-tenantene har ingen policy og ingen tilbud — ingenting sendes.
"""
...
spec = importlib.util.spec_from_file_location(
    "m26_fasit", Path(__file__).with_name("m26_fasit.py"))
...
CREDFILER = {
    ...
    "DISPONIT_PRISBOKSVEIP_URL":
        "/etc/disponit/prisboksveip/DISPONIT_PRISBOKSVEIP_URL",
}
...
def main() -> int:
    ...
    def sveip():
        ...
        v = _koble(dsn["DISPONIT_PRISBOKSVEIP_URL"])
        t0 = time.monotonic()
        try:
