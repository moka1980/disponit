#!/usr/bin/env python3
"""Suiteartefaktet for M-44 (sertifiseringen 16/9).

Kjører HELE testsuiten på staging-verten og skriver resultatet som
artefakt — med M-44s andel NAVNGITT og målt for seg. Andelen er PINNET i
`manifestskjema.M44_SUITE_ANDEL` og leses derfra.
...
BRUK (på verten, fra et FULLT utsjekk, med FERSK testbase):
    ...
    DISPONIT_TEST_KAMPANJESVEIP_DSN=... \
    /opt/disponit/.venv/bin/python deploy/staging/m44-suite-artefakt.py

SVEIPEROLLENS DSN ER PÅKREVD: `test_m44_fasit_port.py` hopper uten den,
...
from manifestskjema import (M44_SUITE_ANDEL,  # noqa: E402
                            _sjekk_grenser, m44_bevisrot_sha256,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        totalt, roede, hoppet, kode = _kjor(["tests"],
                                            Path(tmp) / "alle.xml")
        m44, m44_roede, m44_hoppet, m44_kode = _kjor(
            [str(REPO / sti) for sti in M44_SUITE_ANDEL],
            Path(tmp) / "m44.xml")
    ts = datetime.now(timezone.utc).isoformat()
    art = {"krav_id": "m44-suite-v1", "ts": ts, "bestatt": True,
           "oppsett": {"modul": "m44_kampanje", "commit": commit,
                       "vert": vert, "m44_filer": list(M44_SUITE_ANDEL),
                       "bevisrot_sha256": m44_bevisrot_sha256()},
           "maalt": {"tester_totalt": totalt, "tester_feilet": roede,
                     "tester_hoppet": hoppet,
                     "m44_tester": m44, "m44_feilet": m44_roede,
                     "m44_hoppet": m44_hoppet,
                     "suite_exitkode": kode, "m44_exitkode": m44_kode}}
    formfeil = valider_artefaktformat(art, "m44-suite-v1")
    grensefeil = _sjekk_grenser("m44-suite-v1", art)
    art["bestatt"] = not formfeil and not grensefeil
    ut = a.ut or (
        REPO / "deploy/staging/artefakter"
        / f"m44-suite-v1-{ts[:19].replace(':', '').replace('-', '')}Z.json")
    ...
    print(f"skrev {ut} (bestatt={art['bestatt']},"
          ...
          f" m44-andel {m44 - m44_hoppet} av {m44})")
