#!/usr/bin/env python3
"""Registrer m23_fordring-kjeden gjennom de HERDEDE funksjonene
...
Kjøres på verten når purringsmodulen skal i drift (ARC B, PR 4):

    DISPONIT_MIGRATOR_URL=… python3 deploy/staging/registrer-m23-fordring.py \\
        <release_id> <kontrakt_hash> <artifact_digest> \\
        <payload_skjema_hash> <kvittering_skjema_hash>

Kontraktklassen er `krever_outbox`/`kompenserende`: en purring er sendt
når den er sendt, og bransjemalens reversering er
`purring.beklag_og_korriger` — en ny e-post, ikke en angring.
...
Ingen artefakttype: purringen produserer ingen rapport, kvitteringen ER
...
MANIFEST = Path("platform/modules/m23_fordring/manifest.yaml")
