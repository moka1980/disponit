#!/usr/bin/env python3
"""Registrer m17_kundeservice-kjeden gjennom de HERDEDE funksjonene
...
Kjøres på verten når svarmodulen skal i drift (ARC B kundeservice, PR 4):

    DISPONIT_MIGRATOR_URL=… python3 deploy/staging/registrer-m17-kundeservice.py \\
        <release_id> <kontrakt_hash> <artifact_digest> \\
        <payload_skjema_hash> <kvittering_skjema_hash>

Kontraktklassen er `krever_outbox`/`kompenserende`: et kundesvar er
sendt når det er sendt, og policyens reversering er
`kundeservice.svar.korriger` — en ny e-post, ikke en angring.
...
Ingen artefakttype: svaret produserer ingen rapport, kvitteringen ER
evidensen (sendt_ts, malversjon, melding_id, mottakermaske).
"""
...
MANIFEST = Path("platform/modules/m17_kundeservice/manifest.yaml")
