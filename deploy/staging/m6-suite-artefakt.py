#!/usr/bin/env python3
"""Suiteartefaktet for M-6 (sertifiseringen 16/9, kveld).

Kjører HELE testsuiten på staging-verten og skriver resultatet som
artefakt — med M-6s andel NAVNGITT og målt for seg. Andelen er PINNET i
`manifestskjema.M6_SUITE_ANDEL` og leses derfra.
...
BRUK (på verten, fra et FULLT utsjekk, med FERSK testbase):
    ...
    DISPONIT_TEST_PLAN_DSN=... \
    /opt/disponit/.venv/bin/python deploy/staging/m6-suite-artefakt.py

SVEIPEROLLENS OG PLANARBEIDERENS DSN ER PÅKREVD: `test_m6_fasit_port.py`
...
from manifestskjema import (M6_SUITE_ANDEL,  # noqa: E402
                            _sjekk_grenser, m6_bevisrot_sha256,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        totalt, roede, hoppet, kode = _kjor(["tests"],
                                            Path(tmp) / "alle.xml")
        m6, m6_roede, m6_hoppet, m6_kode = _kjor(
            [str(REPO / sti) for sti in M6_SUITE_ANDEL],
            Path(tmp) / "m6.xml")
    ts = datetime.now(timezone.utc).isoformat()
    art = {"krav_id": "m6-suite-v1", "ts": ts, "bestatt": True,
           "oppsett": {"modul": "m06_epost", "commit": commit,
                       "vert": vert, "m6_filer": list(M6_SUITE_ANDEL),
                       "bevisrot_sha256": m6_bevisrot_sha256()},
           "maalt": {"tester_totalt": totalt, "tester_feilet": roede,
                     "tester_hoppet": hoppet,
                     "m6_tester": m6, "m6_feilet": m6_roede,
                     "m6_hoppet": m6_hoppet,
                     "suite_exitkode": kode, "m6_exitkode": m6_kode}}
    formfeil = valider_artefaktformat(art, "m6-suite-v1")
    grensefeil = _sjekk_grenser("m6-suite-v1", art)
    art["bestatt"] = not formfeil and not grensefeil
    ut = a.ut or (
        REPO / "deploy/staging/artefakter"
        / f"m6-suite-v1-{ts[:19].replace(':', '').replace('-', '')}Z.json")
