...
Delingen har én betingelse, og den er hele forskjellen på deling og smutthull: **notatet må navngi hvilken MÅLING i det delte artefaktet som beviser punktet for nettopp den modulen.** «Samme artefakt» uten «samme måling» er å låne en konklusjon. Eksempler fra de tre manifestene: `rollback-m01-v1` beviser `rollback_testet` for M-2 gjennom `tapte_loggposter = 0` og uendret radtelling for `revisjonslogg` — men **ikke** for M-37, fordi den kjøringen deaktiverte beslutningsmodulen og aldri arbeiderprosessen. M-37s punkt står derfor `nei`.
